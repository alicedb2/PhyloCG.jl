...
mutable struct RAM <: Sampler
    ...
    targetα::Float64
    prevΔα::Float64
    Σ::Cholesky{Float64, <: AbstractMatrix{Float64}}

    rng::AbstractRNG
end

function RAM(model::AbstractString="fbd", rng::Union{Nothing, AbstractRNG}=nothing)

    rng = _getrng(rng)
    params, mask = initmodel(model, randomizewith=rng)
    
    Σ0 = 0.1 * I(length(params))
    Σ = Cholesky(LowerTriangular(Matrix{Float64}(Σ0)))
    ...
    sampler = RAM(
            ...
            1, 0.6, 0.234, 0.234, Σ, rng)

    return sampler
end

function advance!(sampler::RAM, cgtree; maxsubtree=Inf)
    
    s = sampler

    p_new = transform(s.params, s.mask)

    U = randn(s.rng, length(p_new))
    SU = s.Σ.L * U
    SU[.!s.mask] .= 0
    p_new .+= SU
    
    # transform back into original space
    backtransform!(p_new, s.mask)

    logprob_new = logdensity(cgtree, p_new; maxsubtree=maxsubtree)
    
    logacc = logprob_new - s.current_logprob
    
    logacc += log_hastings_ratio(p_new, s.params, s.mask)

    α = isfinite(logacc) ? min(1.0, exp(logacc)) : 0.0
    ...
    s.prevΔα = Δα
    s.i += (s.prevΔα * Δα <= 0)
    ΔS = sqrt(η * abs(Δα)) * SU / norm(U)

    if Δα > 0
        lowrankupdate!(s.Σ, ΔS)
    else
        lowrankdowndate!(s.Σ, ΔS)
    end
...
#             p_new[6:8] .= ηαβ((uvw(s.params.h.eta, s.params.h.alpha, s.params.h.beta) .+ [du, 0.0, 0.0])...)
#         elseif i == 7
#             dv = exp(s.logscales[7]) * randn()
#             p_new[6:8] .= ηαβ((uvw(s.params.h.eta, s.params.h.alpha, s.params.h.beta) .+ [0.0, dv, 0.0])...)
#         elseif i == 8
#             dw = exp(s.logscales[8]) * randn()
#             p_new[6:8] .= ηαβ((uvw(s.params.h.eta, s.params.h.alpha, s.params.h.beta) .+ [0.0, 0.0, dw])...)
#         else
#             p_new[i] += exp(s.logscales[i]) * randn()
#         end
#         accprob = 0.0
#         if p_new.h.eta > 0.0
#             accprob += logjac_dηαβduvw(p_new.h.eta, p_new.h.alpha, p_new.h.beta) - logjac_dηαβduvw(s.params.h.eta, s.params.h.alpha, s.params.h.beta)
#         end
