...
function resample_bouquet(bouquet::Bouquet; check=true, rng=Random.GLOBAL_RNG)::Bouquet

    if check
        all(bouquet.trunk.s .== getfield(bouquet.crown, :t)) || throw(ArgumentError("Trunk and crown are not connected (trunk s must be equall to all t's of crown)"))
        allequal(getfield.(bouquet.crown, :s)) || throw(ArgumentError("Crown subtrees are not of the same depth (must all have same s)"))
    end

    nb_tips = sum(getfield.(bouquet.crown, :k))

    new_partition = randompartitionAD5(nb_tips)

    new_trunk = Subtree(bouquet.trunk.t, bouquet.trunk.s, length(new_partition))
    ct, cs = bouquet.crown.t, bouquet.crown.s 
    new_crown = Subtree[Subtree(ct, cs, k) for k in new_partition]

    return (trunk=new_trunk, crown=new_crown)
